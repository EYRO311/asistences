...
export function WeekPage({ items }: Props) {
    ...
    return (
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <h2 className={`text-sm font-semibold capitalize ${isToday ? "text-foreground" : "text-muted"}`}>
            {label}
          </h2>
          {isToday && (
          ...
          {dayItems.length === 0 && (
            <span className="text-xs text-muted/60">Sin eventos</span>
          )}
        </div>
        {dayItems.map((item) => (
  ...
  return (
    <div className="px-4 pt-8 pb-4 space-y-6">
      <h1 className="font-handwriting text-4xl">Esta semana</h1>
        ...
        <div>
          ...
          {pastOpen && (
            <div className="space-y-6 opacity-60">
              {pastDays.map((day) => (
                <DaySection key={toDateParam(day)} day={day} />
              ))}
            </div>
          )}
        </div>
